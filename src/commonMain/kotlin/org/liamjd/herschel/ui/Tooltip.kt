...
class Tooltip(
	...
	renderer: TextRenderer<String> = WrappableTextRenderer(wrapWidth, wrapAlignment),
	autoScaling: Boolean = true,
) : Container(), ViewLeaf, IText {
	private val textBox =  WrappableText(text, textSize, color, font, alignment, wrapWidth, renderer, autoScaling)
...
inline fun Container.tooltip(
	...
	Tooltip(
		...
		wrapWidth = wrapWidth,
		wrapAlignment = wrapAlignment,
		renderer = WrappableTextRenderer(wrapWidth, wrapAlignment)
	).addTo(container)
...
fun Container.addTooltip(
	...
	val tt = tooltip(text,wrapWidth = this.scaledWidth, textSize = textSize, color = color, backgroundColor = backgroundColor, container = container).apply {
		visible = false
